...
2. *OTA parolası* (ana form): durum satırı `Durum: <b id="ota-state">` → “Tanımlı değil · OTA parolasız” / “Tanımlı” / “Kaydedildi · yeniden başlatma bekliyor” / “Kaldırıldı · yeniden başlatma bekliyor” (bekleyen durum amber). Alanlar `otaPw` Yeni OTA parolası (password 8–64, `off:clearOtaPassword`), `clearOtaPassword` “OTA parolasını kaldır (parolasız OTA)”. İpucu: yalnız özet saklanır, gerekiyorsa yeniden başlatmada geçerli olur, yükleme aracında `--auth` gerekir.
