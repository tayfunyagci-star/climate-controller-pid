...
- OTA parolası web parolasından bağımsızdır. UI kayıtlı/aktif/bekleyen değişimi ayırır, reboot gerekirse söyler.
- **Aile varsayılanı: parolasız OTA açıktır, parolasız durum kalıcı uyarıdır.** Parola Ayarlar › Erişim'deki ayrı “OTA parolası” formundan tanımlanır ve onaylı “Parolayı kaldır” ile iptal edilir; parolasızken hem üst genel uyarı hem panel uyarısı görünür. Proses riski olan cihazda (ör. rezistans yükü) parolasız olsa bile her yüklemede güvenli duruş zorunludur. Profil açıkça parola zorunluluğu isterse bu varsayılan değiştirilir.
- ESP32 ArduinoOTA (Arduino-ESP32 2.0.x): `setPasswordHash`/`setPassword` parola bir kez atandıktan sonra değişikliği sessizce yok sayar ve `end()` parolayı silmez. Parola değişiminde global `ArduinoOTA` yerine kendi `ArduinoOTAClass` örneğini yeniden oluştur; aksi hâlde değişim yeniden başlatmaya kadar etkisizdir.
